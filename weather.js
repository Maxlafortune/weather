...
let handleWeatherResponse = function(response) {
  ...
  // let html = '<div class="row current" style="display:none">';
  // html = html + '<div class="col">';
  // html = html + '<h1 id="current-conditions-icon">' + icon(response.currently.icon) + '</h1>';
  // html = html + '<h1 id="location"></h1>';
  // html = html + '<h2 id="current-conditions-text">' + response.currently.summary + '</h2>';
  // html = html + '</div></div><div class="row forecast" style="display:none">';
  // html = html + '</div>';
